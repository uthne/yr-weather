...
function wpyr_weather_shortcode($atts){
	...
	if ($isloop) {
		...
		if (!$options['wpyr_template_loop_area']) {
			if ($options['wpyr_template_loop_checkbox'] == '1' && $options['wpyr_template_html_area'] && $options['wpyr_template_html_area'] != '') {
	...
	// endre $offset til $i inne i loop

	
    $description    = $forecast[$offset]['symbol']['@attributes']['name'];
	$Description	= wpyr_translate_description($description);
    $symbol         = $forecast[$offset]['symbol']['@attributes']['number'];
    $symbol_name    = $forecast[$offset]['symbol']['@attributes']['var'];
	$symbol_url		= $icon_url.$symbol_name.'.svg';
    $precipitation  = $forecast[$offset]['precipitation']['@attributes']['value'];
    $windDirection  = $forecast[$offset]['windDirection']['@attributes']['code'];// W S E N etc
	$windDirTranlate= wpyr_translate_direction($windDirection);
	$Windtext		= wpyr_describe_direction($windDirection);
    $windDegree     = $forecast[$offset]['windDirection']['@attributes']['deg']; // 260 etc
    $windSpeed      = $forecast[$offset]['windSpeed']['@attributes']['mps']; // mps
    $temp_unit      = $forecast[$offset]['temperature']['@attributes']['unit'];
    $temperature    = $forecast[$offset]['temperature']['@attributes']['value'];
	$temperature_f  = intval((($temperature / 5) * 9) + 32);
    $pressure_scale = $forecast[$offset]['pressure']['@attributes']['unit'];
    $pressure       = $forecast[$offset]['pressure']['@attributes']['value'];
    $time_from      = $forecast[$offset]['@attributes']['from'];
    $time_to        = $forecast[$offset]['@attributes']['to'];
	...
	switch ($offset) {
    	...
    	break;
  		default:
			$Offset_html = $offset . " " . __( 'hours from now','yr-weather' );
	} 
    ...
    $wpyr_out .= wpyr_translate_template($template);
	...
	if ($isloop && $post_template != '') $wpyr_out .= $post_template;
